...
# 6) В /склад добавлена возможность пинговать заранее указанные роли
#    при уведомлениях за 3, 2 и 1 час до сгорания склада.
#
# ВАЖНО:
...
#
# ВАЖНО:
# 1) Если хочешь пинговать КОНКРЕТНЫХ ЛЮДЕЙ — добавляй их Discord ID в SKLAD_PING_USER_IDS.
#    Это самый надёжный вариант, потому что личные упоминания обычно работают без права Mention Everyone.
# 2) Если хочешь пинговать РОЛИ — добавляй ID ролей в SKLAD_PING_ROLE_IDS.
#    Для пинга ролей боту может понадобиться право Mention Everyone,
#    а роль должна быть доступна для упоминания или бот должен иметь право упоминать роли.
SKLAD_PING_USER_IDS = [
    # Пример:
    # 123456789012345678,
]
...
def get_sklad_ping_text(guild):
    """
    Собирает строку пинга для уведомлений склада.

    Поддерживает:
    - конкретных пользователей из SKLAD_PING_USER_IDS;
    - роли из SKLAD_PING_ROLE_IDS.

    Для пользователей бот пишет <@ID>.
    Для ролей бот пишет <@&ID>.
    """
    if not guild:
        return ""
    ...
    for role_id in SKLAD_PING_ROLE_IDS:
        try:
            ...
            logger.warning("Некорректный ID роли склада для пинга: %s", role_id)
            continue
...
async def send_sklad_warning_if_needed(timer_row, now_ts):
    ...
    has_role_pings = bool(SKLAD_PING_ROLE_IDS)
    missing = get_missing_bot_permissions(
        channel,
        guild,
        need_embed=False,
        need_role_mentions=has_role_pings,
    )
    if missing:
        logger.warning(
    ...
    ping_text = get_sklad_ping_text(guild)

    allowed_mentions = discord.AllowedMentions(
        users=True,
        roles=True if SKLAD_PING_ROLE_IDS else False,
        everyone=False,
    )
