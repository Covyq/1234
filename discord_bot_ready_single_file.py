...
logging.basicConfig(
    ...
    handlers=[
        # bot.log будет хранить до 5 файлов по 2 МБ, чтобы логи не росли бесконечно
        RotatingFileHandler(
            "bot.log",
...
def channel_matches(ctx_channel, configured_channel_id):
    """
    Разрешает работу как в обычном канале, так и в ветке.
    Если команда вызвана в ветке, проверяем ID ветки.
    """
    return ctx_channel and ctx_channel.id == configured_channel_id
...
async def loop():
    ...
    # Предупреждения для складов, которые ещё не сгорели
